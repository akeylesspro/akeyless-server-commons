...
export const query_document_optional = async (collection_path, field_name, operator, value) => {
    try {
        const querySnapshot = await db.collection(collection_path).where(field_name, operator, value).get();
        const documentsData = querySnapshot.docs;
        const documents = documentsData.flatMap((doc) => simple_extract_data(doc));
...
export const set_document = async (collection_path, doc_id, data) => {
    try {
        await db
...
export const add_document = async (collection_path, data, include_id = false) => {
    try {
...
/// token
export const verify_token = async (bearer_token) => {
    try {
...
export const init_snapshots = async () => {
    await snapshot_bulk([
        snapshot({
...
export const snapshot_bulk_by_names = async (params) => {
    const start = performance.now();
