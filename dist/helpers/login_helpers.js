...
export const get_users_by_phone = async (phone_number, default_client_key = DefaultClient.default) => {
    let installer;
    let nx_user;
    const installer_q = await query_document_optional("technicians", "phone", "==", convert_to_short_phone_number(phone_number));
    const nx_user_q = await query_document_optional("nx-users", "phone_number", "==", convert_to_short_phone_number(phone_number));
