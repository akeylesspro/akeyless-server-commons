...
const send_local_sms = async (recepient, text) => {
...
const send_international_sms = async (recepient, text) => {
...
const send_iccid_sms = async (recepient, text) => {
    const { sms_provider: { monogoto }, } = cache_manager.getObjectData("nx-settings");
...
export const send_sms = async (recepient, text, entity_for_audit) => {
    try {
        const send = async () => {
...
const keep_outgoing_sms = async (recepient, content, service, external_id) => {
