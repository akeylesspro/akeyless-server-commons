...
const parse__add_update__settings = (documents: any[], name_for_cache: string): void => {
    const data: TObject<any> = default_cache_manager.get(name_for_cache, {});
    documents.forEach((doc: TObject<any>) => {
        data[doc.id] = doc;
    });
    default_cache_manager.set(name_for_cache, data);
};
...
const parse_add_update__as_object = (documents: any[], config: OnSnapshotConfig, doc_key_property: string): void => {
    const data: TObject<any> = default_cache_manager.get(config.collection_name, {});
    documents.forEach((doc: TObject<any>) => {
        data[doc[doc_key_property]] = doc;
    });
    default_cache_manager.set(doc_key_property, data);
};
...
const parse__add_update__as_array = (documents: any[], config: OnSnapshotConfig): void => {
    ...
    const existing_array: any[] = default_cache_manager.get(config.collection_name);
